...
import 'dart:convert';
import 'dart:io';
import 'package:embla/src/http/http_exceptions.dart';
...
.hljs-strong {
  ...
  Future<Response> catchError(error, StackTrace stack) async {
    ...
    return new Response(
      code,
      body: renderError(error, stack, code),
      headers: {
  ...
  Stream<List<int>> renderError(error, StackTrace stack, int code) {
    return _template(error, new Chain.forTrace(stack), code).map(UTF8.encode);
  }
